...
void* YoloObjectDetector::displayInThread(void* ptr) {
  int c = show_image(buff_[(buffIndex_ + 1) % 3], "Object Detection", 1);
  if (c != -1) c = c % 256;
  if (c == 27) {
    demoDone_ = 1;
    return 0;
...
void YoloObjectDetector::yolo() {
  ...
  if (!demoPrefix_ && viewImage_) {
    ...
    if (fullScreen_) {
      cv::setWindowProperty("Object Detection", cv::WND_PROP_FULLSCREEN, cv::WINDOW_FULLSCREEN);
    } else {
      cv::moveWindow("Object Detection", 0, 0);
      cv::resizeWindow("Object Detection", 640, 360);
    }
  }
